...
import org.hammerlab.shapeless.coproduct.Cast.Ops
import shapeless._
...
object Cast {
  ...
  class Ops[T, L <: HList](t: T)(implicit c: Cast.Aux[T, L]) {
...
trait HasCast {
  ...
  type Cast[T] = coproduct.Cast[T]
   val Cast    = coproduct.Cast
  @inline implicit def ShapelessCoproductCastOps[T, L <: HList](t: T)(implicit c: Cast.Aux[T, L]): Ops[T, L] = new Ops(t)
}
